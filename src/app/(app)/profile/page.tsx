  ...
  return (
    <div className="flex flex-1 flex-col px-4 py-8 md:px-6 md:py-10">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <Badge variant="default" className="mt-2">{content.roles[role] ?? role}</Badge>
      ...
      <div className="mt-8 rounded-xl border border-border bg-card p-5">
        <div className="grid grid-cols-3 divide-x divide-border">
          <div className="flex flex-col items-center px-2">
            <span className="text-xl font-bold tabular-nums text-foreground">{stats.totalRides}</span>
            <p className="text-sm font-medium text-muted-foreground mt-1.5 text-center">{content.stats.totalRides}</p>
          </div>
          <div className="flex flex-col items-center px-2">
            <span className="text-xl font-bold tabular-nums text-foreground">{stats.ridesThisMonth}</span>
            <p className="text-sm font-medium text-muted-foreground mt-1.5 text-center">{content.stats.thisMonth}</p>
          </div>
          <div className="flex flex-col items-center px-2">
            <span className="text-xl font-bold tabular-nums text-foreground">{memberSince}</span>
            <p className="text-sm font-medium text-muted-foreground mt-1.5 text-center">{content.sections.memberSince}</p>
          </div>
        </div>
      </div>
      ...
      {/* About */}
      <div className="mt-8">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{content.sections.about}</h2>
        {profile.bio ? (
      ...
      {profile.preferred_pace_group && (
        <div className="mt-8">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{content.sections.preferences}</h2>
          <div className="mt-3">
      ...
      {/* Recent Rides */}
      <div className="mt-8">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{content.recentRides}</h2>
