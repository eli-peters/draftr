...
import { getUserClubMembership, getMeetingLocations, getPaceGroups, getClubTags, getRideById, getRideTagIds } from "@/lib/rides/queries";
...
export default async function CreateRidePage({
  ...
  const [meetingLocations, paceGroups, tags, sourceRide, sourceTagIds, templateData] = await Promise.all([
    ...
    templateId
      ? supabase.from("ride_templates").select("*").eq("id", templateId).single().then((r) => r.data)
      : null,
  ]);
  ...
  if (sourceRide) {
    ...
    initialData = {
      ...
      distance_km: templateData.default_distance_km != null ? String(templateData.default_distance_km) : "",
      elevation_m: "",
